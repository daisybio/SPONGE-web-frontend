export const DISEASE_GENE_TYPES: { by_version_and_disease: Record<string, string[]>; by_disease: Record<string, string[]>; all_types: string[] } = {
  "by_version_and_disease": {
    "1:::bladder urothelial carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::brain lower grade glioma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::breast invasive carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::cervical & endocervical cancer": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "1:::colon adenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::esophageal carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "1:::head & neck squamous cell carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::kidney renal clear cell carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "rRNA_pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::kidney renal papillary cell carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "1:::liver hepatocellular carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::lung adenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::lung squamous cell carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::ovarian serous cystadenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "rRNA_pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "1:::pancancer": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::pancreatic adenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "1:::pheochromocytoma & paraganglioma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "1:::prostate adenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::sarcoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::stomach adenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "1:::testicular germ cell tumor": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "rRNA_pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "1:::thymoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::thyroid carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "1:::uterine corpus endometrial carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::acute myeloid leukemia": [
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_LoF",
      "sense_intronic",
      "transcribed_unprocessed_pseudogene"
    ],
    "2:::adrenocortical carcinoma": [
      "processed_pseudogene",
      "protein_coding",
      "protein_coding_CDS_not_defined"
    ],
    "2:::bladder urothelial carcinoma": [
      "TEC",
      "antisense",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::brain lower grade glioma": [
      "TEC",
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::breast invasive carcinoma": [
      "TEC",
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::cervical & endocervical cancer": [
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "sense_overlapping",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene"
    ],
    "2:::cholangiocarcinoma": [
      "antisense",
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::colon adenocarcinoma": [
      "antisense",
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::esophageal carcinoma": [
      "TEC",
      "antisense",
      "antisense_RNA",
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::head & neck squamous cell carcinoma": [
      "TEC",
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "nonsense_mediated_decay",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::kidney chromophobe": [
      "antisense",
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_overlapping"
    ],
    "2:::kidney renal clear cell carcinoma": [
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::kidney renal papillary cell carcinoma": [
      "TEC",
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::liver hepatocellular carcinoma": [
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene"
    ],
    "2:::lung adenocarcinoma": [
      "artifact",
      "lncRNA",
      "protein_coding",
      "retained_intron",
      "sense_overlapping",
      "transcribed_unitary_pseudogene"
    ],
    "2:::lung squamous cell carcinoma": [
      "antisense",
      "artifact",
      "lincRNA",
      "lncRNA",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene"
    ],
    "2:::lymphoid neoplasm diffuse large b-cell lymphoma": [
      "antisense",
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::mesothelioma": [
      "artifact",
      "protein_coding"
    ],
    "2:::ovarian serous cystadenocarcinoma": [
      "artifact",
      "lncRNA",
      "protein_coding"
    ],
    "2:::pancancer": [
      "TEC",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "non_stop_decay",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::pancreatic adenocarcinoma": [
      "antisense",
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene"
    ],
    "2:::parkinsons disease": [
      "lincRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron"
    ],
    "2:::pheochromocytoma & paraganglioma": [
      "nonsense_mediated_decay",
      "processed_pseudogene",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron"
    ],
    "2:::prostate adenocarcinoma": [
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::rectum adenocarcinoma": [
      "artifact",
      "nonsense_mediated_decay",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "transcribed_unitary_pseudogene"
    ],
    "2:::sarcoma": [
      "TEC",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::skin cutaneous melanoma": [
      "antisense",
      "artifact",
      "lincRNA",
      "lncRNA",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron"
    ],
    "2:::stomach adenocarcinoma": [
      "TEC",
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::testicular germ cell tumor": [
      "TEC",
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::thymoma": [
      "TEC",
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "non_stop_decay",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::thyroid carcinoma": [
      "TEC",
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "2:::uterine carcinosarcoma": [
      "lincRNA",
      "nonsense_mediated_decay",
      "protein_coding",
      "retained_intron"
    ],
    "2:::uterine corpus endometrial carcinoma": [
      "antisense",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene"
    ],
    "2:::uveal melanoma": [
      "antisense",
      "lincRNA",
      "lncRNA",
      "processed_transcript",
      "protein_coding",
      "retained_intron"
    ]
  },
  "by_disease": {
    "acute myeloid leukemia": [
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_LoF",
      "sense_intronic",
      "transcribed_unprocessed_pseudogene"
    ],
    "adrenocortical carcinoma": [
      "processed_pseudogene",
      "protein_coding",
      "protein_coding_CDS_not_defined"
    ],
    "bladder urothelial carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "brain lower grade glioma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "breast invasive carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "cervical & endocervical cancer": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "cholangiocarcinoma": [
      "antisense",
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "sense_intronic",
      "sense_overlapping",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "colon adenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "esophageal carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "head & neck squamous cell carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "kidney chromophobe": [
      "antisense",
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "sense_overlapping"
    ],
    "kidney renal clear cell carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "rRNA_pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "kidney renal papillary cell carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "liver hepatocellular carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "lung adenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "retained_intron",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "lung squamous cell carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "lymphoid neoplasm diffuse large b-cell lymphoma": [
      "antisense",
      "artifact",
      "lincRNA",
      "lncRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "retained_intron",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unprocessed_pseudogene"
    ],
    "mesothelioma": [
      "artifact",
      "protein_coding"
    ],
    "ovarian serous cystadenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "misc_RNA",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "pseudogene",
      "rRNA_pseudogene",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "pancancer": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "misc_RNA",
      "non_stop_decay",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "pancreatic adenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "parkinsons disease": [
      "lincRNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron"
    ],
    "pheochromocytoma & paraganglioma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "miRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "prostate adenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "rectum adenocarcinoma": [
      "artifact",
      "nonsense_mediated_decay",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron",
      "transcribed_unitary_pseudogene"
    ],
    "sarcoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "skin cutaneous melanoma": [
      "antisense",
      "artifact",
      "lincRNA",
      "lncRNA",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "retained_intron"
    ],
    "stomach adenocarcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "miRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "testicular germ cell tumor": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "macro_lncRNA",
      "miRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "pseudogene",
      "rRNA_pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene",
      "vaultRNA"
    ],
    "thymoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "non_stop_decay",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "thyroid carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snRNA",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "translated_processed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "uterine carcinosarcoma": [
      "lincRNA",
      "nonsense_mediated_decay",
      "protein_coding",
      "retained_intron"
    ],
    "uterine corpus endometrial carcinoma": [
      "3prime_overlapping_ncRNA",
      "IG_C_gene",
      "IG_C_pseudogene",
      "IG_V_gene",
      "IG_V_pseudogene",
      "TEC",
      "TR_C_gene",
      "TR_V_gene",
      "TR_V_pseudogene",
      "antisense",
      "antisense_RNA",
      "artifact",
      "bidirectional_promoter_lncRNA",
      "lincRNA",
      "lncRNA",
      "miRNA",
      "misc_RNA",
      "nonsense_mediated_decay",
      "polymorphic_pseudogene",
      "processed_pseudogene",
      "processed_transcript",
      "protein_coding",
      "protein_coding_CDS_not_defined",
      "protein_coding_LoF",
      "pseudogene",
      "retained_intron",
      "scRNA",
      "scaRNA",
      "sense_intronic",
      "sense_overlapping",
      "snoRNA",
      "transcribed_processed_pseudogene",
      "transcribed_unitary_pseudogene",
      "transcribed_unprocessed_pseudogene",
      "unitary_pseudogene",
      "unprocessed_pseudogene"
    ],
    "uveal melanoma": [
      "antisense",
      "lincRNA",
      "lncRNA",
      "processed_transcript",
      "protein_coding",
      "retained_intron"
    ]
  },
  "all_types": [
    "3prime_overlapping_ncRNA",
    "IG_C_gene",
    "IG_C_pseudogene",
    "IG_V_gene",
    "IG_V_pseudogene",
    "TEC",
    "TR_C_gene",
    "TR_V_gene",
    "TR_V_pseudogene",
    "antisense",
    "antisense_RNA",
    "artifact",
    "bidirectional_promoter_lncRNA",
    "lincRNA",
    "lncRNA",
    "macro_lncRNA",
    "miRNA",
    "misc_RNA",
    "non_stop_decay",
    "nonsense_mediated_decay",
    "polymorphic_pseudogene",
    "processed_pseudogene",
    "processed_transcript",
    "protein_coding",
    "protein_coding_CDS_not_defined",
    "protein_coding_LoF",
    "pseudogene",
    "rRNA_pseudogene",
    "retained_intron",
    "scRNA",
    "scaRNA",
    "sense_intronic",
    "sense_overlapping",
    "snRNA",
    "snoRNA",
    "transcribed_processed_pseudogene",
    "transcribed_unitary_pseudogene",
    "transcribed_unprocessed_pseudogene",
    "translated_processed_pseudogene",
    "unitary_pseudogene",
    "unprocessed_pseudogene",
    "vaultRNA"
  ]
};
